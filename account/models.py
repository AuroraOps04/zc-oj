...
class UserManager(BaseUserManager):
    ...
    def create_superuser(self, username, password):
        user = self.create(username=username, email='test@test.com')
...
class User(AbstractBaseUser):
    """
        扩展 AbstractBaseUser 和 AbstractUser 的区别:
        AbstractBaseUser 只提供三个属性:
            1. password
            2. last_login
            3. is_active
        扩展的 AbstractBaseUser 必须指定 USERNAME_FIELD, REQUIRED_FIELDS
            USERNAME_FIELD 必须设置。 设置认证标识，设置成标识的字段 unique=True
            REQUIRED_FIELDS 必须设置。当通过createsuperuser管理命令创建一个用户时，用于提示的一个字段名称列表。
    """
